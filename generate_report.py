...
def generate_pdf(interval="weekly"):
    ...
    story.append(Paragraph(
        ...
        f"<b>Generated On:</b> {datetime.date.today().strftime('%B %d, %Y')}", meta_style))
    ...
    if not student_logs:
        ...
        for student_name, logs in student_logs.items():
            student_section = []
            student_section.append(Paragraph(
                f"<b>Student:</b> {html.escape(student_name)} &ndash; <i>{len(logs)} commit(s)</i>", sub_section_style))
